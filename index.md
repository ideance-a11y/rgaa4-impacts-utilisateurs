...
Les personnes aveugles ou malvoyantes ne peuvent pas lire le code d’un captcha visuel. Les personnes handicapées mentales ou cognitives peuvent ne pas comprendre un captcha visuel ou logique.  
...
Certaines personnes présentant des troubles de la lecture (Dyslexie par exemple) peuvent avoir recours à des feuilles de styles personnalisées pour modifier la police d'écriture et en utiliser une plus spécifique. Les textes en images ne peuvent être adaptés dans ce cas.
...
Beaucoup de personnes ont une vision partielle ou dégradée des couleurs. Lorsqu'il n'est pas possible de distinguer une information véhiculée par la couleur, ces personnes perdent de l'information. Par exemple un champ erroné signalé uniquement par le remplacement de la bordure en rouge.
...
Les personnes sourdes ou malentendantes auront du mal à suivre une vidéo si :
...
Pour ces personnes, l'absence (ou le manque de pertinence) d’une alternative textuelle empêche l’accès à l’information contenue dans le média.
...
### Critères 5.6, 5.7 - Identification des en-têtes et liaisons des cellules d’en-têtes et de données
...
Les personnes déficientes motrices ne pourront que très difficilement utiliser ses composants qui peuvent nécessiter des interactions riches au clavier.
...
### Critère 7.5 – Restitution des messages de statut
...
Des contenus informatifs insérés via CSS (image de fond véhiculant de l’information par exemple) peuvent ne pas être restitués par les lecteurs d’écran ou les systèmes de loupes vocalisés.
...
### Critère 10.6 – Visibilité des liens par rapport au texte environnant
...
Les personnes aveugles ou malvoyantes qui utilisent un lecteur d’écran ont un accès séquentiel à l’information. Si des contenus importants à la compréhension sont cachés, ces personnes rencontreront des difficultés majeures pour les comprendre ou les contrôler.
...
Les personnes déficientes motrices qui utilisent des reconnaissances vocales ainsi que les personnes aveugles et malvoyantes qui utilisent un lecteur d’écran pourraient être dans l’incapacité de renseigner les champs lorsque le contenu des étiquettes est absent.
...
Les personnes aveugles, malvoyantes ou déficientes visuelles qui utilisent un lecteur d’écran ou une loupe vocalisée, pourraient rencontrer des difficultés majeures si les messages d’erreur ne sont pas vocalisés préalablement à la saisie, ou s’ils ne sont pas correctement liés aux champs correspondants.

Les personnes handicapées mentales pourraient être dans l’incapacité de remplir un champ de formulaire sans indication préalable du format attendu ou fourniture d’un exemple de saisie réelle en cas d’erreur persistante.

Les utilisateurs pourraient être dans l’incapacité de soumettre un formulaire lorsque les champs à caractère obligatoire ne sont pas indiqués.

Les personnes handicapées mentales pourraient être dans l’incapacité de remplir un champ de formulaire sans un exemple de saisie réelle en cas d’erreur.

Les personnes handicapées mentales pourraient être dans l’incapacité de remplir un champ de formulaire sans indication préalable du format attendu ou sans un exemple de saisie réelle en cas d’erreur.
...
Les personnes déficientes motrices mettront beaucoup de temps à remplir le formulaire si la liste des champs obligatoires n’est pas donnée dans l’étiquette ou le passage de texte associé, mais en fin de formulaire par exemple.

Les personnes handicapées mentales peuvent être incapables de remplir un champ de formulaire si le format attendu n’est pas précisé.

Les personnes handicapées mentales et les personnes aveugles ou malvoyantes qui utilisent un lecteur d’écran ou une loupe vocalisée peuvent être incapables d’envoyer un formulaire si les champs obligatoires ne sont pas indiqués avant le bouton d’envoi.

Les personnes aveugles ou malvoyantes qui utilisent un lecteur d’écran ou une loupe vocalisée, peuvent avoir de grandes difficultés à envoyer un formulaire si :

* Le message d’erreur n’est pas vocalisé avant le champ en erreur ;
* Ou si le message d’erreur et le champ ne sont pas reliés.
...
Après avoir envoyé un formulaire, les personnes handicapées mentales pourraient ne pas comprendre le message d’erreur affichée si :

* Le format du champ en erreur n’est pas précisé ;
* Une valeur n’est pas donnée en exemple.

</div>

### Critère 11.12 – Droit à l’erreur
...
Les personnes déficientes motrice et les personnes ayant des difficultés de lecture peuvent saisir des données incorrectes dans un formulaire. L’impossibilité d'annuler des actions, de revoir et de corriger les informations renseignées pourrait avoir de graves conséquences.

</div>
<div class="impact-falc">

Certaines personnes présentant des troubles de l’attention ou ayant des difficultés de lecture et toutes les personnes ayant des problèmes d’agilité peuvent saisir des données incorrectes. S’il n’est pas possible de revoir et de corriger les données ou d’annuler l’envoi du formulaire, cela pourrait avoir de graves conséquences.
...
Les personnes handicapées mentales et déficientes motrices pourraient rencontrer des difficultés à renseigner les champs d’un formulaire si leurs données utilisateur préalablement enregistrées ne sont pas suggérées. Également, les technologies d’assistance ont la possibilité de présenter par des icônes la nature d’un champ ce qui permet aux personnes rencontrant des difficultés de lecture de mieux comprendre la saisie attendue. Par exemple, une icône représentant un téléphone pour un champ de saisie avec un attribut autocomplete="tel".

Un champ autocomplété avec une valeur incorrecte peut potentiellement renseigner une donnée utilisateur erronée et obliger les personnes à devoir modifier la valeur du champ manuellement.

</div>
<div class="impact-falc">

Les personnes handicapées mentales et les personnes déficientes motrices peuvent avoir des difficultés à renseigner les champs d’un formulaire si leurs données personnelles ne sont pas suggérées.

De même, un champ dont la liste des données proposées ne correspond pas à la valeur attendue pose également problème : Cela oblige les personnes à devoir modifier la valeur du champ manuellement.
...
* Critère 12.3. La [page « plan du site »](https://accessibilite.numerique.gouv.fr/methode/glossaire/#page-plan-du-site) est-elle pertinente ?

</div>

De manière générale, tous les utilisateurs pourraient rencontrer des difficultés dans le cas où la navigation dans le site ou un ensemble de pages ne dépend que d’un système de navigation unique.

Pour les personnes déficientes motrices les difficultés pourraient devenir majeures s’ils ne sont pas en capacité d’utiliser un menu de navigation trop complexe en l’absence de système de navigation alternatif, comme un plan du site par exemple.

Pour les personnes handicapées mentaux ou cognitifs, déficientes motrices, aveugles et malvoyantes, la navigation dans le site peut être rendue laborieuse ou complexe en l’absence d’information d’orientation comme l’indication de la page active dans les menus de navigation, la mise à disposition d’un fil d’Ariane ou la non-identification des systèmes de navigation.
...
Les personnes handicapées choisissent la manière de naviguer qui répond le mieux à leurs besoins :

* Les personnes aveugles ou malvoyantes peuvent trouver plus simple d’utiliser la recherche plutôt que de faire défiler le contenu dans une loupe d’écran ou d’écouter le contenu restitué par un lecteur d’écran ;
* Les personnes handicapées mentales ou cognitives peuvent préférer l’utilisation d’un plan du site qui donne une vue d'ensemble ou trouver plus facile de passer par la fonction de recherche ;
* Certaines personnes peuvent choisir d’explorer le site de manière séquentielle (en passant d'une page à l'autre) afin de mieux comprendre la présentation ;
* …
...
Les personnes aveugles ou malvoyantes pourraient avoir du mal à trouver les systèmes de navigation s’ils ne sont pas toujours placés au même endroit dans le code HTML ou s’ils ne sont pas clairement identifiables.

Les personnes handicapées mentales ou cognitives pourraient avoir du mal à voir les systèmes de navigation s’ils n’ont pas le même style sur les différentes pages du site.
...
Les personnes déficientes motrices et les personnes handicapées mentales ou cognitives pourraient avoir de grandes difficultés pour naviguer dans le site si le système de navigation est trop complexe et que le plan du site n’est pas fonctionnel : S’il contient des liens morts ou qu’il ne permet pas d’atteindre facilement l’ensemble des pages du site, par exemple.

</div>

### Critères 12.4, 12.5 – Systèmes de navigation atteignables de manière identique
...
Les personnes aveugles et malvoyantes pourraient rencontrer des difficultés pour repérer les éléments de navigation si ceux-ci sont déplacés d’une page à l’autre dans le code de la page.
...
### Critères 12.6, 12.7 – Accès rapide aux contenus
...
Les personnes déficientes motrices, aveugles ou malvoyantes qui utilisent le clavier pourraient ne pas pouvoir naviguer rapidement vers la zone principale du contenu lorsque le lien d’accès rapide n’est pas fonctionnel.

Les personnes déficientes visuelles utilisant un lecteur d’écran pourraient être dans l’incapacité d’atteindre ou évitées les zones de regroupement de contenus principales.
...
Les personnes aveugles ou malvoyantes qui utilisent un lecteur d’écran et les personnes déficientes motrice ne peuvent pas naviguer entre les différentes zones de contenu facilement.
...
Les personnes déficientes motrices qui naviguent au clavier ne peuvent pas atteindre la zone de contenu principale en un minimum d’efforts.

</div>

### Critère 12.8 – Ordre de tabulation cohérente
...
L'ordre de tabulation est déterminant à la fois dans l’exploration des contenus, mais surtout dans la manipulation d’éléments complexes ou dans l’enchaînement d’actions successives. Tous les utilisateurs naviguant au clavier vont rencontrer des difficultés importantes si celui-ci n’est pas correct ou logique.

Les handicapés moteurs qui naviguent au clavier peuvent être fortement impactés si les liens d’accès rapides ne sont pas fonctionnels.

</div>
<div class="impact-falc">

Les personnes aveugles, malvoyantes ou déficientes motrices qui naviguent au clavier parcourent le contenu dans l’ordre du code source : Un ordre de tabulation incorrect rend très difficile la découverte du contenu, la manipulation des éléments et la réalisation de certaines actions.
...
Les utilisateurs de clavier peuvent être bloqués dans la navigation d’une page lorsqu’ils rencontrent des pièges au clavier. Dans ce cas, il n'est plus possible pour l’utilisateur de parcourir la page et d’accéder au contenu.

</div>
<div class="impact-falc">

Face à un piège au clavier, les personnes aveugles ou malvoyantes qui utilisent un lecteur d’écran et les personnes déficientes motrices et ne peuvent plus déplacer le focus : Elles sont bloquées et sont obligées de quitter la page pour récupérer le contrôle du curseur.

</div>

### Critère 12.10 – Raccourcis clavier contrôlables
...
Les personnes naviguant à la voix ainsi que les personnes ayant de problèmes de dextérité pourraient malencontreusement activer une fonctionnalité qui utilise un raccourci à touche unique s’ils ne peuvent pas désactiver ou modifier ces raccourcis.

</div>
<div class="impact-falc">

Sans un moyen de contrôler les raccourcis clavier à une touche, les personnes déficientes motrices naviguant à la voix et toutes les personnes ayant des problèmes d’agilité pourraient activer un raccourci par erreur et démarrer une fonction non voulue.
...
Les personnes n’utilisant pas une souris comme les personnes aveugles, malvoyantes ou déficientes motrices pourraient rencontrer des difficultés ou pourraient être dans l’incapacité de contrôler des contenus qui apparaissent au survol, à la prise de focus ou à l’activation d’un composant d’interface s’ils ne sont pas atteignables au clavier.

</div>
<div class="impact-falc">

Les personnes aveugles, malvoyantes ou déficientes motrices qui naviguent au clavier peuvent être dans l’incapacité d’atteindre et de contrôler le contenu qui s’affiche à la suite d’une action.
...
Le recours à des technologies d’assistance (lecteur d’écran ou périphériques spécialisés) et les conséquences des déficiences subies par les utilisateurs impactent directement les performances dans l’exploration, la lecture et l’utilisation des contenus et fonctionnalités.

La modification des contenus à intervalles réguliers, l’absence de contrôle des processus de mise à jour ou les limites de temps fixées pour réaliser une tâche, ont toujours comme conséquence que ces utilisateurs puissent se retrouver dans l’impossibilité d’accomplir une tâche ou, simplement, de lire le contenu.

</div>
<div class="impact-falc">

L’utilisation de technologies d’assistance (comme un lecteur d’écran, une loupe vocalisée ou un périphérique spécialisé) et les effets de certaines déficiences ralentissent fortement la découverte du contenu, la manipulation des éléments et la réalisation de certaines actions…

Toutes les personnes utilisant une technologie d’assistance ou atteintes d’une déficience auront du mal à accomplir une tâche ou à lire le contenu, si elles non pas la possibilité d’arrêter (ou de contrôler) les délais de mise à jour.

</div>

### Critère 13.2 – Ouverture d’une nouvelle fenêtre inattendue
...
Les personnes souffrant de troubles cognitifs et les personnes utilisant des lecteurs d'écran ou des agrandisseurs d'écran peuvent être déroutées par un événement inattendu tel que l’ouverture d’une nouvelle fenêtre.

</div>
<div class="impact-falc">

Les personnes handicapées mentales ou cognitives et les personnes aveugles ou malvoyantes qui utilisent un lecteur ou un agrandisseur d’écran peuvent être désorientés par l’ouverture non prévue d’une nouvelle fenêtre.

</div>

### Critères 13.3, 13.4 – Documents bureautiques en téléchargement
...
Les personnes aveugles ou malvoyantes vont avoir de grandes difficultés si les documents en téléchargement ne sont pas traités pour l’accessibilité ou ne disposent pas d’alternative accessible.

</div>
<div class="impact-falc">

Les personnes en situation de handicap, notamment les personnes déficientes visuel (cécité, malvoyance, daltonisme entre autres) ou encore les personnes déficientes cognitives (présentant par exemple des troubles Dys) auront de grandes difficultés pour consulter un document en téléchargement qui n’est pas accessible ou pour lequel il n’existe pas d’alternative accessible.

</div>

### Critères 13.5, 13.6 – Alternatives des contenus cryptiques
...
Émoticônes, texte cryptique ou art ASCII ne représentent dans la plupart des cas qu'une succession de caractères pour les personnes aveugles et malvoyantes. Ces derniers ne peuvent accéder à la représentation visuelle de ces successions de caractères.

</div>
<div class="impact-falc">

Les personnes aveugles ou malvoyantes qui utilisent un lecteur d’écran ne visualisent pas la forme ou le dessin représenté par une émoticône, du texte cryptique ou de l’art ASCII :  
Ces éléments sont restitués par le lecteur d’écran comme une succession de caractères sans signification particulière.

</div>

### Critères 13.7, 13.8 - Accessibilité des contenus dynamiques
...
Un contenu en mouvement ne peut pas être lu par des personnes rencontrant des difficultés de lecture, s’il défile trop rapidement. Il peut distraire les personnes ayant un déficit de l’attention et les empêcher de lire le reste du texte de la page.
...
Les flashs et les changements brusques de luminosités peuvent rendre la lecture d’un contenu impossible pour les personnes malvoyantes (comme les achromates).

Ces effets visuels peuvent aussi provoquer des crises d’épilepsie chez certaines personnes sensibles à la lumière.
...
La présence de texte en mouvement peut rendre l’accès au contenu très difficile. C’est le cas pour :

* Certaines personnes présentant des troubles de la lecture (comme la dyslexie) : Elles ne peuvent pas lire le texte s’il défile trop rapidement ;
* Certaines personnes présentant des troubles de l’attention : Elles peuvent être distraites et ne pas arriver à lire le reste du contenu ;
* Certaines personnes utilisant un lecteur d’écran : Un texte en mouvement peut empêcher le lecteur d’écran de restituer le contenu.

</div>

### Critère 13.9 – Contenus consultables quelle que soit l’orientation de l’écran
...
Certaines personnes déficientes motrices, comme une personne atteinte de tétraplégie, utilisent leur appareil installé dans une position fixe et n’ont pas la capacité à modifier l’orientation de celui-ci. Dans le cas ou la page web serait disponible uniquement dans une orientation spécifique, ces personnes seraient dans l’incapacité de percevoir et d’interagir correctement avec l’interface.

</div>
<div class="impact-falc">

Certaines personnes déficientes motrices (comme une personne tétraplégique, par exemple) n’ont pas la possibilité de modifier l’orientation de l’écran qu’elles utilisent. Si la page web est disponible dans un seul format, ces personnes ne peuvent pas visualiser et interagir correctement avec l’interface.

</div>

### Critère 13.10 – Alternative au geste complexe
...
Les personnes déficientes motrices rencontrant des difficultés pour enchaîner plusieurs actions, pour utiliser des combinaisons de touches, ou pour contrôler des mouvements sont dans l’incapacité d’effectuer des gestes complexes.

</div>
<div class="impact-falc">

Les personnes déficientes motrices et toutes les personnes ayant des problèmes d’agilité peuvent ne pas arriver à réaliser des gestes complexes. Par exemple : Faire plusieurs actions en même temps, utiliser des combinaisons de touches, ou encore exécuter une suite de gestes particuliers.

</div>

### Critère 13.11 – Annulation des actions déclenchées au moyen d’un dispositif de pointage
...
Les personnes déficientes motrices peuvent déclencher par erreur des évènements liés au toucher, à la souris ou tout système de pointage. Ces personnes seraient dans l’incapacité d’utiliser une interface si les actions déclenchées par erreur ne peuvent être annulées.

</div>
<div class="impact-falc">

Les personnes malvoyantes, déficientes motrices et handicapées mentales ou cognitives peuvent activer des fonctionnalités ou exécuter des commandes liées au toucher, à la souris ou à tout autre système de pointage sans le vouloir. S’il n’est pas possible d’annuler l’action, l’interface pourrait devenir inutilisable.

</div>

### Critère 13.12 – Alternative aux mouvements de l’appareil
...
Les personnes déficientes motrices ne pouvant incliner, secouer ou réaliser un mouvement spécifique de l’appareil pourraient être dans l’incapacité de réaliser des actions si une alternative par un autre moyen accessible n’est pas proposée.

</div>
<div class="impact-falc">

Les personnes déficientes motrices peuvent être dans l’incapacité de réaliser un mouvement spécifique de l’appareil (comme l’incliner ou le secouer). Si aucun autre moyen n’est proposé pour démarrer la fonction, ces personnes n’y auront pas accès.

À l’inverse, certaines personnes déficientes motrices peuvent activer des fonctions sans le vouloir (à cause de tremblements, par exemple). S’il n’est pas possible d’annuler l’action, l’interface pourrait devenir inutilisable.
